...
function ResetPassword(){
  ...
  async function save(e){
    ...
    if(!supabase){setError('Supabase authentication is not available.');return;}
    setBusy(true);
    const {error}=await supabase.auth.updateUser({password});
    if(error) setError(error.message);
    else { setMessage('Password updated. You can sign in with your new password.'); setPassword(''); setConfirm(''); setTimeout(()=>navigate('/'),700); }
    setBusy(false);
  }
  return <div className="login"><div className="login-card"><div className="brand large">HLL // COMMAND<small>CLAN OPERATIONS HUB</small></div><div className="eyebrow">ACCOUNT RECOVERY</div><h1>RESET PASSWORD</h1><p>Choose a new password for your command account.</p><form onSubmit={save} className="stack"><label>New password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} minLength={8} required autoComplete="new-password"/></label><label>Confirm password<input type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} minLength={8} required autoComplete="new-password"/></label>{error&&<div className="error">{error}</div>}{message&&<div className="callout">{message}</div>}<button className="btn primary" disabled={busy}>{busy?'SAVING…':'SAVE NEW PASSWORD'} <KeyRound size={15}/></button></form></div></div>
}
...
function Login(){
  ...
  async function forgotPassword(){
    if(!supabase){setError('Connect Supabase first.');return;}
  ...
  return <div className="login"><div className="login-card"><div className="brand large">HLL // COMMAND<small>CLAN OPERATIONS HUB</small></div><div className="eyebrow">SECURE ACCESS</div><h1>COMMAND LOGIN</h1><p>Sign in to access operations, stage maps, rosters and briefings.</p><form onSubmit={submit} className="stack"><label>Email<input value={email} onChange={e=>setEmail(e.target.value)} placeholder="name@clan.com" required autoComplete="email"/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} minLength={8} required autoComplete="current-password"/></label>{error&&<div className="error">{error}</div>}{resetSent&&<div className="callout">Password reset email sent. Check your inbox and open the link.</div>}<button className="btn primary" disabled={busy}>{busy?'AUTHENTICATING…':'SIGN IN'} <LogIn size={15}/></button><button type="button" className="btn" disabled={busy} onClick={forgotPassword}>FORGOT PASSWORD?</button><button type="button" className="btn" disabled={busy} onClick={signup}>CREATE ACCOUNT</button></form><div className="login-foot">{supabase?'Supabase authentication enabled':'Demo mode enabled — add Supabase env vars to activate accounts.'}</div></div></div>
}
...
function CommandRoom({clan,user,data}){
  const navigate=useNavigate();
  ...
  useEffect(()=>{
    if(!supabase||!clan?.id){setActivity([]);setLoading(false);return;}
    let live=true;
    async function load(){
      const {data:rows}=await supabase.from('clan_activity').select('id,event_type,title,message,created_at,operation_id').eq('clan_id',clan.id).order('created_at',{ascending:false}).limit(10);
      if(live){setActivity(rows||[]);setLoading(false);}
    }
    load();
    const ch=supabase.channel(`command-room-${clan.id}`).on('postgres_changes',{event:'INSERT',schema:'public',table:'clan_activity',filter:`clan_id=eq.${clan.id}`},p=>setActivity(prev=>[p.new,...prev].slice(0,10))).subscribe();
    return ()=>{live=false;supabase.removeChannel(ch)};
  },[clan?.id]);
  if(!op) return <><PageHead eyebrow="COMMAND ROOM" title="STANDBY" subtitle="NO OPERATION HAS BEEN CREATED"/><div className="card section empty-state"><Radio size={24}/><h2>No active operation</h2><p className="muted">Create an operation to open the command console.</p>{command&&<button className="btn primary" onClick={()=>navigate('/operations')}><Plus size={15}/> NEW OPERATION</button>}</div></>;
  ...
  const assigned=new Set(squads.flatMap(s=>s.playerIds||[]));
  const unassigned=going.filter(p=>!assigned.has(p.id));
  const briefs=op.briefingsByPlayer||{};
  const published=going.filter(p=>briefs[p.id]?.published).length;
  const acknowledged=going.filter(p=>briefs[p.id]?.acknowledged || op.briefingReceipts?.[briefs[p.id]?.id]?.acknowledged).length;
  const phases=op.strategyData?.phases||[];
  const strategyReady=!!op.strategyData?.intent&&!!op.strategyData?.orders&&phases.length>=4&&phases.every(p=>p.intent&&p.tasks?.length);
  const maps=op.stageMaps||[];
  const mapsReady=maps.length>=4&&maps.every(m=>(m.markers||[]).length>0);
  const alerts=[];
  if(responded.length<players.length) alerts.push({tone:'yellow',title:'Attendance incomplete',detail:`${players.length-responded.length} players have not responded.`});
  if(unassigned.length) alerts.push({tone:'red',title:'Unassigned going players',detail:`${unassigned.length} going player${unassigned.length===1?'':'s'} still need a squad.`});
  if(!strategyReady) alerts.push({tone:'yellow',title:'Strategy incomplete',detail:'Intent, orders, phases or tasks still need attention.'});
  if(!mapsReady) alerts.push({tone:'yellow',title:'Stage maps incomplete',detail:'At least one battle phase has no tactical map markers.'});
  if(published<going.length) alerts.push({tone:'yellow',title:'Briefings pending',detail:`${going.length-published} going players have no published briefing.`});
  if(published && acknowledged<published) alerts.push({tone:'yellow',title:'Orders awaiting acknowledgement',detail:`${published-acknowledged} published briefings are not acknowledged.`});
  if(!alerts.length) alerts.push({tone:'green',title:'Command board green',detail:'No outstanding readiness alerts.'});
  const next=alerts.find(a=>a.tone==='red')||alerts.find(a=>a.tone==='yellow')||alerts[0];
  return <>
    <PageHead eyebrow="COMMAND / LIVE CONTROL" title="OPERATIONS COMMAND CONSOLE" subtitle={`OPERATION #${op.id} · ${op.name} · ${op.map} · VS ${op.opponent}`} actions={<><Tag tone={op.status==='active'?'green':'yellow'}>{op.status.toUpperCase()}</Tag><button className="btn primary" onClick={()=>navigate(`/operations/${op.id}`)}>OPEN OPERATION <ArrowUpRight size={14}/></button></>}/>
    <div className="grid g4"><Stat label="GOING" value={going.length} sub={`${responded.length}/${players.length} RESPONDED`} trend={going.length>0}/><Stat label="UNASSIGNED" value={unassigned.length} sub="GOING WITHOUT SQUAD" trend={unassigned.length===0}/><Stat label="BRIEFINGS" value={`${published}/${going.length||0}`} sub={`${acknowledged} ACKNOWLEDGED`} trend={published===going.length&&going.length>0}/><Stat label="STRATEGY" value={strategyReady?'READY':'PENDING'} sub={mapsReady?'MAPS READY':'MAPS PENDING'} trend={strategyReady&&mapsReady}/></div>
    <div className="grid g2 section">
      <div className="card section"><div className="section-head"><div><h3>Live squad board</h3><small>COMMAND VIEW · LAST-MINUTE CHANGES ALLOWED</small></div><button className="btn" onClick={()=>navigate(`/operations/${op.id}`)}>MANAGE</button></div><div className="command-squad-grid">{squads.map(s=>{const members=players.filter(p=>(s.playerIds||[]).includes(p.id));const lead=players.find(p=>p.id===s.lead||p.memberUserId===s.lead||p.name===s.lead);const ready=members.filter(p=>attendance[p.id]==='going');return <div className="command-squad" key={s.id}><div className="command-squad-head"><div><b>{s.name.toUpperCase()}</b><small>{lead?`SL · ${lead.name}`:'NO SL ASSIGNED'}</small></div><Tag tone={ready.length===members.length&&members.length?'green':'yellow'}>{ready.length}/{members.length}</Tag></div><div className="command-player-list">{members.length?members.map(p=><div key={p.id} className="command-player"><span>{p.name}</span><Tag tone={attendance[p.id]==='going'?'green':attendance[p.id]==='maybe'?'yellow':'red'}>{(attendance[p.id]||'PENDING').toUpperCase()}</Tag></div>):<span className="muted">EMPTY SQUAD</span>}</div></div>})}</div>{unassigned.length>0&&<div className="callout danger"><AlertTriangle size={15}/><b>{unassigned.length} GOING PLAYER{unassigned.length===1?'':'S'} UNASSIGNED</b> · <button className="linkbtn" onClick={()=>navigate(`/operations/${op.id}`)}>OPEN SQUAD BOARD</button></div>}</div>
      <div className="card section"><div className="section-head"><div><h3>Command alerts</h3><small>NEXT ACTION FIRST</small></div><Tag tone={next.tone}>{next.tone==='green'?'GREEN':next.tone==='red'?'CRITICAL':'ATTENTION'}</Tag></div><div className="alert-stack">{alerts.slice(0,6).map((a,i)=><div className={`command-alert ${a.tone}`} key={i}><div><b>{a.title}</b><small>{a.detail}</small></div><ChevronDown size={14}/></div>)}</div><div className="next-action"><div className="eyebrow">NEXT COMMAND ACTION</div><h3>{next.title}</h3><p>{next.detail}</p><button className="btn primary" onClick={()=>navigate(`/operations/${op.id}`)}>OPEN OPERATION WORKSPACE</button></div></div>
    </div>
    <div className="grid g3 section"><div className="card"><div className="section-head"><h3>Mission phases</h3><span>{phases.filter(p=>p.intent&&p.tasks?.length).length}/{phases.length||4} READY</span></div><div className="side-list">{(phases.length?phases:DEFAULT_PHASES).map(p=><div className="row" key={p.id}><div><b>{String(p.no).padStart(2,'0')} — {p.name}</b><small>{p.intent||'No intent defined.'}</small></div><Tag tone={p.intent&&p.tasks?.length?'green':'yellow'}>{p.intent&&p.tasks?.length?'READY':'PENDING'}</Tag></div>)}</div></div><div className="card"><div className="section-head"><h3>Delivery</h3><span>PLAYER ORDERS</span></div><div className="metric-list"><div><span>PUBLISHED</span><b>{published}/{going.length||0}</b></div><div><span>ACKNOWLEDGED</span><b>{acknowledged}/{published||0}</b></div><div><span>READINESS</span><b>{going.length?Math.round((acknowledged/going.length)*100):0}%</b></div></div><button className="btn" onClick={()=>navigate(`/operations/${op.id}`)}>OPEN BRIEFINGS</button></div><div className="card"><div className="section-head"><h3>Map coverage</h3><span>TACTICAL PHASES</span></div><div className="metric-list">{maps.map(m=><div key={m.phaseNo}><span>PHASE {String(m.phaseNo).padStart(2,'0')} · {m.name}</span><b>{m.markers?.length||0}</b></div>)}</div><button className="btn" onClick={()=>navigate(`/operations/${op.id}`)}>OPEN STAGE MAPS</button></div></div>
    <div className="card section"><div className="section-head"><div><h3>Live command feed</h3><small>REALTIME EVENTS FOR THIS CLAN</small></div><button className="btn" onClick={()=>navigate('/activity')}>FULL FEED</button></div>{loading?<div className="empty-state"><p className="muted">Loading live feed…</p></div>:activity.length?<div className="activity-list compact-feed">{activity.map(item=><div className="activity-item" key={item.id}><div className="activity-dot"></div><div className="activity-main"><div className="activity-top"><Tag>{item.event_type.replaceAll('_',' ').toUpperCase()}</Tag><time>{new Date(item.created_at).toLocaleString()}</time></div><b>{item.title}</b><p>{item.message||'Command activity recorded.'}</p></div></div>)}</div>:<div className="empty-state"><Activity size={20}/><p className="muted">No command events recorded yet.</p></div>}</div>
  </>;
}
...
function MembershipWaiting({clan,user}){
  ...
  return <div className="splash"><Shield size={36}/><div><div className="eyebrow">MEMBERSHIP REQUEST</div><h2>AWAITING COMMAND APPROVAL</h2><small>{clan?.name||'Clan'} · {user?.email}</small><p className="muted">Your account is waiting for a Commander or CO to approve your membership. The page will check automatically.</p><button className="btn primary" onClick={check} disabled={checking}>{checking?'CHECKING…':'CHECK STATUS'}</button><button className="btn" onClick={async()=>{if(supabase) await supabase.auth.signOut(); else window.location.reload();}}><LogOut size={15}/> SIGN OUT</button></div></div>}
...
function MemberProfile({clan,user,data}){
  ...
  useEffect(()=>{
    ...
    async function load(){
      ...
      const {data:row,error:e}=await supabase.from('clan_members').select('id,user_id,callsign,primary_role,role,active,membership_status,squad_preference,command_notes,created_at').eq('id',memberId).eq('clan_id',clan.id).single();
      if(!live)return;
      if(e){setError(e.message||'Member not found.');setMember(null);setLoading(false);return;}
      setMember(row); setCallsign(row.callsign||''); setPrimaryRole(row.primary_role||'Rifleman'); setAccessRole(row.role||'player'); setSquadPreference(row.squad_preference||''); setCommandNotes(row.command_notes||''); setLoading(false);
    }
    load();
    return ()=>{live=false};
  ...
  async function save(e){
    ...
    try{
      ...
      const {data:updated,error:e}=await supabase.from('clan_members').update(patch).eq('id',member.id).eq('clan_id',clan.id).select('id,user_id,callsign,primary_role,role,active,membership_status,squad_preference,command_notes,created_at').single();
      if(e)throw e;
  ...
  return <>
    <PageHead eyebrow="PERSONNEL COMMAND" title={member.callsign||'MEMBER PROFILE'} subtitle="IDENTITY · ROLE · STATUS · COMMAND NOTES" actions={<button className="btn" onClick={()=>navigate('/members')}><ArrowLeft size={15}/> BACK TO MEMBERS</button>}/>
    <div className="grid g3">
    ...
    <div className="grid g2">
      ...
      <div className="card form">
        ...
        {admin&&<div className="callout"><Shield size={15}/> Only Commander can edit command notes. Other members can view their own profile without seeing private notes.</div>}
        {!admin&&<div className="callout"><Shield size={15}/> Command notes are managed by the Commander.</div>}
      </div>
    </div>
  </>
}
...
function ClanSettings({clan,setClan}){
  ...
  const [message,setMessage]=useState('');
  const [error,setError]=useState('');
  useEffect(()=>{setName(clan?.name||'');setTag(clan?.tag||'')},[clan?.id,clan?.name,clan?.tag]);
  async function save(e){
    e.preventDefault(); setMessage(''); setError('');
    const cleanName=name.trim(); const cleanTag=tag.trim().toUpperCase();
    if(!cleanName||!cleanTag){setError('Clan name and tag are required.');return;}
    if(cleanTag.length>8){setError('Clan tag must be 8 characters or fewer.');return;}
    setBusy(true);
    try{
      if(!supabase){setClan(c=>c?{...c,name:cleanName,tag:cleanTag}:c);setMessage('Clan settings saved.');return;}
      const {data:updated,error:e}=await supabase.from('clans').update({name:cleanName,tag:cleanTag,updated_at:new Date().toISOString()}).eq('id',clan.id).select('id,name,tag,invite_code').single();
      if(e)throw e;
      setClan(c=>c?{...c,name:updated.name,tag:updated.tag,inviteCode:updated.invite_code||c.inviteCode}:c);
      await logActivity(clan.id,'clan_settings','Clan settings updated',`${cleanName} · ${cleanTag}`,null,null,{});
      setMessage('Clan settings saved.');
    }catch(err){setError(err.message||'Could not save clan settings.');}finally{setBusy(false);}
  }
  return <><PageHead eyebrow="COMMAND ADMIN" title="CLAN SETTINGS" subtitle="IDENTITY · INVITES · COMMAND CONFIGURATION"/><div className="grid g2"><div className="card form"><div className="eyebrow">CLAN IDENTITY</div><h2>PUBLIC CLAN NAME</h2><p className="subtitle">This information appears in the command header and member workspace.</p><form onSubmit={save} className="stack"><label className="field"><span>CLAN NAME</span><input value={name} onChange={e=>setName(e.target.value)} maxLength={80} required/></label><label className="field"><span>CLAN TAG</span><input value={tag} onChange={e=>setTag(e.target.value.toUpperCase())} maxLength={8} required/></label>{error&&<div className="error">{error}</div>}{message&&<div className="success">{message}</div>}<button className="btn primary" disabled={busy}>{busy?'SAVING…':'SAVE CLAN SETTINGS'} <Save size={15}/></button></form></div><div className="card"><div className="eyebrow">CURRENT CONFIGURATION</div><h2>{clan?.name||'CLAN'}</h2><div className="status-line"><Tag tone="green">{clan?.tag||'TAG'}</Tag><Tag>{(clan?.role||'player').toUpperCase()}</Tag></div><div className="callout"><Shield size={15}/> Only Commander and CO accounts can change the clan identity.</div></div></div></>
}
