...
function SquadHub({clan,user}){
  const [squads,setSquads]=useState([]); const [members,setMembers]=useState([]); const [assignments,setAssignments]=useState([]); const [preferences,setPreferences]=useState([]); const [loading,setLoading]=useState(true); const [error,setError]=useState(''); const [busy,setBusy]=useState(false);
  const command=canCommand(clan); const canManage=canManageSquads(clan);
  async function load(){
    if(!supabase||!clan?.id){setLoading(false);return;}
    setLoading(true); setError('');
    try{
      const [{data:s,error:se},{data:m,error:me},{data:a,error:ae},{data:p,error:pe}]=await Promise.all([
        supabase.from('clan_squads').select('id,name,short_code,color,squad_lead_id,active,sort_order').eq('clan_id',clan.id).order('sort_order').order('name'),
        ...
        supabase.from('clan_squad_members').select('id,squad_id,user_id').eq('clan_id',clan.id),
        supabase.from('clan_member_preferences').select('member_id,squad_preference').eq('clan_id',clan.id)
      ]);
      if(se)throw se; if(me)throw me; if(ae)throw ae; if(pe)throw pe;
  ...
  async function setLead(squadId,userId){
  ...
  return <>
    ...
    <div className="grid g4">
      ...
      <Stat label="ACTIVE MEMBERS" value={members.length} sub="AVAILABLE FOR ASSIGNMENT"/>
